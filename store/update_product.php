    ...
    if(isset($_POST["update_product"])){
        ...
        if (is_array($_FILES['image_product']['name']) || is_object($_FILES['image_product']['name']))
        {
            foreach($_FILES['image_product']['name'] as $key=>$val)
            {
                ...
                move_uploaded_file($image_product_tmpN,'../avatar/'.$image);
                $dataImg .=$image." ";
            }
            ...
            {
                $query="UPDATE `produit` SET `name_product`='$name_product',`description`='$description',`etat`='$etat',`qnt`='$qnt',`promotion`='$promotion',`ID_category`='$categoriy',`price`='$prix',`images`='$dataImg',`ID_user`='$id_owner_store' WHERE ID_produit='$ID_product'";
                $result=mysqli_query($conx,$query);
                if($result){
                    $_SESSION["success"]="la modification avec success";
                    header("location:product");
                }else{
